...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(index => {
    let testNum = index.reduce((newValue, CurrentValue) => {
      if(CurrentValue % 5 === 0 && typeof(CurrentValue) === 'number'){
        newValue.push(Math.pow(2, CurrentValue));
      }
      return newValue;
    }, []);
    newArr.push(testNum);
